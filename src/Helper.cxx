...
TGraph* generateGraph(std::string name, std::string xTitle,
	...
	for (unsigned int i = 0; i < yValues.size() - 1; i++) {
		a += abs(yValues[i + 1] - yValues[i])
				/ abs(xValues[i + 1] - xValues[i]);
		b += yValues[i] - a / (double) (i + 1) * xValues[i];
	}
	a /= yValues.size() - 1;
	b /= yValues.size() - 1;
	...
	TF1 f1;
	if (fitQuadratic) {
		f1 = TF1("f1", "[0]+[1]*x+[2]*x^2", fitRangeStart, fitRangeEnd);
		f1.SetParName(2, "quad.");
		f1.SetParameter(2, -0.0002);
//		f1.SetParLimits(2, 0.01 * -0.0002, 100 * -0.0002);
	} else {
		f1 = TF1("f1", "pol1", fitRangeStart, fitRangeEnd);
	}
	...
	f1.SetParameter(0, b);
	f1.SetParameter(1, a);
	f1.SetParLimits(0, 0.8 * b, 1.2 * b);
	f1.SetParLimits(1, 0.8 * a, 1.3 * a);
		...
		for (auto& VaAndDg : EdAndVa.second) {
			...
			graphs.push_back(
					generateGraph(title.str(), "DriftGap [mm]", driftGaps, 0.1,
							HitWidths, HitWidthErrors, 0, 100, true,
							lineColor++));
